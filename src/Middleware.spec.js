...
describe("Middleware", () => {
	...
	describe("run()", () => {
		...
		it("에러 발생시 에러 미들웨어만 실행한다", () => {
			...
			sinon.stub(stub, "mw1").callsFake((req, res, next) => next());
			sinon
				.stub(stub, "mwWillThrow")
				.callsFake((req, res, next) => next(Error()));
			sinon.stub(stub, "mw2").callsFake((req, res, next) => next());
			sinon
				.stub(stub, "mwWillCatchError")
				.callsFake((err, req, res, next) => null);

			const fns = [stub.mw1, stub.mwWillThrow, stub.mw2, stub.mwWillCatchError];
			fns.forEach((fn) => middleware.add(fn));
